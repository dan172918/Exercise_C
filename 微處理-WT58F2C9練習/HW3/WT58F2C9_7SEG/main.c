...
int main()
{
	...
	while(1){
		...
		if (tmp == 0x00000001){
			n=7;
			for(i=0; i<8; i++){
				for (myi=0; myi<20000; myi++){
					for(j=0; j<=i; j++){
						GPIO_PTA_GPIO = index_7LED[n+j];
						GPIO_PTD_GPIO = index_7LED_NUM[myId[8-j]];
						delay1(10);
					}
		...
		else if(tmp == 0x00000002){
			n=0;
			for(i=0; i<8; i++){
				for (myi=0; myi<20000; myi++){
					for(j=0; j<=i; j++){
						GPIO_PTA_GPIO = index_7LED[n-j];
						GPIO_PTD_GPIO = index_7LED_NUM[myId[j]];
						delay1(10);
					}
				}
				n++;
			...
			for(i=0; i<8; i++){
				for(j=0; j<8-i; j++){
					for (myi=0; myi<20000; myi++){
						...
						GPIO_PTD_GPIO = index_7LED_NUM[myId[8-i]];
						delay1(5);
						for(k=0; k<i; k++){
							GPIO_PTA_GPIO = index_7LED[k];
							GPIO_PTD_GPIO = index_7LED_NUM[myId[8-k]];
							delay1(5);
						}
					}
				}
		...
		else if(tmp == 0x00000008){
			for(i=0; i<8; i++){
				for(j=0; j<8-i; j++){
					for (myi=0; myi<20000; myi++){
						GPIO_PTA_GPIO = index_7LED[j];
						GPIO_PTD_GPIO = index_7LED_NUM[myId[i]];
						delay1(5);
						for(k=0; k<i; k++){
							GPIO_PTA_GPIO = index_7LED[7-k];
							GPIO_PTD_GPIO = index_7LED_NUM[myId[k]];
							delay1(5);
						}
					}
				}
